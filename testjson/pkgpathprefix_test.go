...
func TestRelativePackagePath(t *testing.T) {
	prefix := "github.com/ngavinsir/gotestsum/testjson"
...
func TestGetPkgPathPrefix(t *testing.T) {
	t.Run("with go path", func(t *testing.T) {
		...
		assert.Equal(t, getPkgPathPrefix(), "github.com/ngavinsir/gotestsum/testjson")
	})
	t.Run("with go modules", func(t *testing.T) {
		skip.If(t, !isGoModuleEnabled())
		assert.Equal(t, getPkgPathPrefix(), "github.com/ngavinsir/gotestsum")
	})
}
